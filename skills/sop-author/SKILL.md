...
| 5 自檢 | 逐份檢查：必要節齊（做什麼、誰可以用、輸入或明寫「無」、輸出、步驟**每步**有「怎麼做」——逐步點名檢查，不要掃過去、可能出什麼錯、測試例子）＋測試例子覆蓋規則（成功≥1；開始前確認、出錯、每步出錯欄各條有對應例子）＋**選填節盤點**：把範本全部選填節（特別名詞與分類／相關系統與資料／開始前先確認／怎麼算成功／範圍外／已知的怪現象）逐節寫下「留」或「刪（第幾輪問過、PM 說沒有）」——說不出哪一輪問的＝沒問過＝不准刪。任何一項缺 → 回 Step 3 補問 |
