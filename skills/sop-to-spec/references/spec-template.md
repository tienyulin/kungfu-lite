...
**文件語氣鐵律**：spec 是一份正式文件——**文件裡不寫「給誰看」**（不出現「給審批者」
「給實作 agent」「白話」「Part A/B」這類讀者標籤或後設說明）。讀者分工只存在於
本模板的寫作指引：概要六節用日常語言（禁止 EARS/JSON/狀態機術語），規格各節精確到
零猜測——但成品讀起來就是一份文件，不是兩份附錄。
...
寫作要求（不寫進文件）：
- 全用日常語言；禁止 EARS/JSON/狀態機術語。不懂技術的人讀完概要要能說出
  「這 API 做什麼、最危險的操作是什麼、什麼情況會被擋」。
- 「典型情境」至少三個，必含「風險最高的那個操作」的完整流程＋一個被擋下的失敗情境；
...
情境一「<名稱>」：
- Given <現況>
- When 呼叫 <端點>（先 dry_run 檢查，確認無誤後實際執行）
- Then <結果>，audit 留下紀錄
...
- 本 API 不防護的事項：<殘餘風險逐條，例：兩人同時操作不會互相擋、
  非同步進度需自行查詢>
...
簽核本文件即同意「端點一覽」的端點範圍、「安全防護」的防護等級、
「人工保留項」的保留項目。
```
