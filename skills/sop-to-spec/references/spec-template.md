...
一位不懂技術的人讀前三節就能了解這個 API 做什麼、風險在哪。文字自然；
業界慣用英文表達的技術名詞（如 endpoint、request、schema）保留英文，
其餘照 SOP 的語言。
...
| 不可逆 | SOP 寫「回不去」 | request 必填 `confirm`（固定字串，SCREAMING_SNAKE 格式，例 `CONFIRM_DROP_TABLESPACE`），缺席或不符回 428。SOP 明寫需要核准的，另要求核准單號，驗證方式照 SOP「相關系統與資料」的簽核系統；SOP 沒寫就不加 |

SOP 沒有這節才依內文判斷（出現「無法復原」等字樣視為不可逆），並把判定依據
...
（見第 4 節共通規範）。不懂技術的人讀完這節與 Endpoint 一覽，要能說出
...
確認檢查（428，僅不可逆）、狀態與前置條件（409），最後執行與驗證。
不可逆操作的規則寫在這裡：`confirm` 值不符或缺席回 428；SOP 有要求核准單號的，
單號缺席或驗證未通過也回 428。
...
假設一律取較嚴格的方向。不可逆操作的關鍵參數（confirm 條件、核准要求）不得自行假設，
必須留在未決事項等人補。
